...
def draw_gaze(frame, bbox, pitch, yaw, thickness=2, color=(0, 0, 255)):
    ...
    # THE BELOW SECTION HAS BEEN ADDED BY ME.

    gaze_inside = x_min <= point2[0] <= x_max and y_min <= point2[1] <= y_max
    cv2.putText(frame, "Yes" if gaze_inside else "No", (x_min, y_min - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 255), 2)
    return gaze_inside
...
def draw_bbox_gaze(frame: np.ndarray, bbox, pitch, yaw):
    # THE BELOW SECTION HAS BEEN ADDED BY ME.
    """Draw the bounding box and the gaze."""
    draw_bbox(frame, bbox)
    tof = draw_gaze(frame, bbox, pitch, yaw)
    return tof
